...
export default function PanelLateral({
  ...
  return (
    <>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 scroll-smooth">
              {isLoading ? (
                ...
                <div className="text-center py-12 px-4">
                  ...
                  <p className="text-sm text-[#5e6f79]">
                    Prueba ajustando los filtros o mueve el mapa.
                  </p>
                </div>
              ) : (
                experts.map((expert) => {
                    ...
                    >
                      <div className="flex gap-3 items-center">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <h3 className="font-bold text-[#0d1c2e] text-sm leading-tight mt-1 line-clamp-1">
                            {expert.nombre_completo}
                          </h3>
                          <p className="text-xs text-[#5e6f79] font-medium line-clamp-1">
                            ...
                            {expert.distancia_km && (
                              <>
                                ...
                                <div className="flex items-center gap-0.5 text-[#5e6f79]">
                                  <MapPin size={10} />
                                  <span className="text-xs">{expert.distancia_km} km</span>
                                </div>
                              </>
                            )}
                            {expert.tarifa && (
                        ...
                        <ChevronRight
                          size={16}
                          className={`text-gray-300 transition-all flex-shrink-0 ${isSelected ? "text-[#0288D1] translate-x-1" : "group-hover:text-[#0288D1] group-hover:translate-x-0.5"}`}
                        />
                      </div>
                    </motion.div>
                  );
      ...
      {/* Mobile bottom sheet — shows selected expert count */}
